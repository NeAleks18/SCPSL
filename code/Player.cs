...
partial class SandboxPlayer : Player
{
	...
	private static readonly int[] GuardClothing =
	{
	...
	private static readonly int[] ClassDClothing =
	{
		-394380878, // Prison Jumpsuit.
		591027714 // Sneakers.
	};
	
	private static readonly int[] ScientistClothing =
	{
		-1659713462, // Mutton Chops
		1593736732, // Balding Brown
		-394668142, // Lab Coat
		-1895029643, // Buttoned Blue Shirt // Buttoned Shirt - 1875643260
		-1207323092, // Blue Rubber Gloves
		1194605997, // Smart Trousers
		-1377292782, // Black Boots
	};
	...
	public void UpdateClothes( IClient cl )
	{
		...
		var player = cl.Pawn as SandboxPlayer;
		...
		if (player.Tags.Has("Security"))
		{
			foreach ( var itemId in GuardClothing )
			{
				Clothing.Toggle( ResourceLibrary.Get<Clothing>( itemId ) );
			}
		}
		else if (player.Tags.Has( "ClassD" ))
		{
			foreach ( var itemId in ClassDClothing )
			{
				Clothing.Toggle( ResourceLibrary.Get<Clothing>( itemId ) );
			}
		}
		else if ( player.Tags.Has( "Scientist" ) )
		{
			foreach ( var itemId in ScientistClothing )
			{
				Clothing.Toggle( ResourceLibrary.Get<Clothing>( itemId ) );
			}
		}
